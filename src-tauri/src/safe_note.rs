...
mod tests {
    ...
    fn create_temp_safe_note_file(content: &str) -> PathBuf {
        let temp_dir = std::env::temp_dir();
        let file_path = temp_dir.join("test_safe_note.json");
    ...
    #[test]
    fn test_safe_note_file_structure() {
        let json_content = r#"{"records":[{"title":"encrypted_title","description":"encrypted_desc"}]}"#;
        let safenote: SafeNoteFile = serde_json::from_str(json_content).unwrap();
        assert_eq!(safenote.records.len(), 1);
        assert_eq!(safenote.records[0].title, "encrypted_title");
    }

    #[test]
    fn test_safe_note_into_passages() {
        let safenote = SafeNoteFile {
            records: vec![SafeNoteRecord {
                title: "Test Title".to_string(),
                description: "Test Description".to_string(),
            }],
        };
        let passages = safenote.into_passages();
        assert_eq!(passages.len(), 1);
        assert_eq!(passages[0].title, "Test Title");
        assert_eq!(passages[0].content, "Test Description");
    }

    #[test]
    fn test_safe_note_into_plaintext() {
        let safenote = SafeNoteFile {
            records: vec![
                SafeNoteRecord {
        ...
        let plaintext = safenote.into_plaintext();
